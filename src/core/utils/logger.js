...
const {
    ...
    guildJoin,
    guildLeave,
} = require("../../config/logging-config.json");
...
module.exports = {
    ...
    command(ctx) {
        if (commands) {
            ...
            beautify.log(
                formatText(
                    `${prefix}${ctx.member.displayName} (${ctx.member.id}) used ${ctx.commandName} in ${ctx.guild.name} (${ctx.guild.id})`
                )
            );
        }
    },
    ...
    guildJoin(guild) {
        if (guildJoin) {
            ...
            beautify.log(
                formatText(
                    `${prefix}Joined (${guild.name}) (${guild.id}), members: ${guild.memberCount}`
                )
            );
        }
    },
    ...
    guildLeave(guild) {
        if (guildLeave) {
            ...
            beautify.log(
                formatText(
                    `${prefix}Left (${guild.name}) (${guild.id}), members: ${guild.memberCount}`
                )
            );
        }
    },
